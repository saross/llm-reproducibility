...
| 2026-08-17 | **D3 block run and closed**: probe gate (S4 → named retreat, conditionals to reconciliation layer; P2 pass); three arms sequential under the run contract. Arm 1 sonnet 15/15 clean after adjudicating two verifier defects (hook-spill delivery reads; old-run discovery fallback); arm 2 opus clean first pass; arm 3 fable 13/15 + 2 contract-mandated re-runs (fabricated pull declaration; session-dir Glob — see failure register), accepted at 7.10M tokens with the tripwire event documented. **Wire rule amended (Shawn): remediation-scaled — wire grows by the re-run percentage unless a stuck/repeating-loop indicator is present.** Effort/harness capture added to run records (operator directive); effort was NOT pinned this cycle — session-inherited xhigh, attested | Shawn (rulings); Claude (execution) |
| 2026-08-17 | **Effort-pinning build LANDED** (`3f4cde6`): launcher v1.2 (required `--effort`; `launch_commit` embedded; dirty-tracked-tree refusal, no bypass), workflow v1.5 (effort in scoring opts + Provenance line in every scoring prompt — the harness persists no effort field in spawn metadata, so the transcript prompt is the durable carrier), assembler v1.4 (artefact-derived `provenance_pinned` block; mixed-vintage hard error; `--expect-effort`/`--expect-launch-commit`), 17 new tests incl. cross-file format-contract checks. Tests 222 → 239 green; gate 66/66 PASS | Claude (build) |
| 2026-08-17 | **Effort-study delta pre-run review RULED** (Q1–Q7): output home `outputs/validation/effort-study-2026-08-17/` with **effort in arm identity** (`arm-sonnet-5-max/` etc.; a selected pinned arm's effort enters the D4 arm-choice record); wire = 12M provisional for sonnet@max, opus wire derived empirically (2× opus-xhigh actual × measured sonnet max/xhigh inflation ratio), remediation-scaled rule carries; governance = dated design note + this entry, **no OSF amendment** (exploratory instrument development) — with Shawn's rider that **effort→cost is itself a study measurand**, to be demonstrated empirically, not presumed; comparability mitigations both adopted (session `/effort` set to target as fallback-equals-pin + telemetry-contrast step in the summary verification pass — residual R1: the pin is a request, served effort is verified only indirectly); conditional high arms operator-ruled after max results (esp. opus, costs at scale); no new probe spawns (v1.5 changes fail fast pre-spend; S4/P2 conditions unchanged). D3 contract H1–H15 otherwise carried verbatim. Design note: `studies/open-science-compliance/outputs/validation/effort-study-2026-08-17/design-note.md` — includes the corrected cost estimate (≈$80–120 API-equivalent two arms; the carried ≈$15–25 omitted the cache-write premium and read volume) | Shawn (rulings); Claude (capture) |
| 2026-08-17 | **Effort study GO under the standing API gate** at the corrected ≈$80–120 figure; **mitigation 1 amended on the spot**: the CLI could not switch the main session to `max` (cache invalidation), so fallback-equals-pin is replaced by **P3 — a differential effort probe** (2 sonnet-5 spawns, identical prompt, opts `low` vs `max`; transcript recount must show a large thinking/output differential, else halt pre-spend) — an active verification of the pin channel, strictly before arm 1; session effort stays `xhigh`, attested honestly. GO covers P3 + sonnet-5@max (12M wire); opus-5@max launches only on operator acceptance at the per-arm hard stop | Shawn (GO + amendment acceptance); Claude (execution) |
| 2026-08-17 | **Sonnet@max HALT adjudicated — completion RULED**: 4/15 items unusable + wire tripped at 17.93M/12M; dominant mechanism = max-effort emissions exceeding the 64K per-response harness cap (13 dead retry transcripts, 24% of spend). Ruling: raise `CLAUDE_CODE_MAX_OUTPUT_TOKENS` to 128,000 (project settings; spawn inheritance live-verified, probe P5) and re-run the 4 items — re-run spend on explicit operator continuation authorisation. **Sonnet@high arm ADDED** after max completes (pins the other end of the effort spectrum); **opus@max OFF unless the xhigh run shows deficiencies**. Register entries F-008–F-011 lodged (cap×verbosity loops; workflow null-guard; exploratory Globs with the pattern-vs-path verifier split left open; S4-retreat class caught by C9 as designed). Harness: workflow v1.6 (items re-run filter + null-guard fix), launcher v1.3 (`--items`), assembler v1.5 (declared split provenance `--allow-launch-commits`); tests 239 → 246 | Shawn (rulings); Claude (build + execution) |
| 2026-08-17 | **Sonnet@max COMPLETE — stability 130/150 = 0.8667, below the ≥0.90 gate** (+2 items over xhigh at 3.4× token cost, 21.25M; completion report + run record committed `1f68108`; key r2 needed three attempts; 128K cap eliminated retry churn — F-008 confirmed by intervention). **Opus@high arm ADDED (Shawn)**: runs after sonnet@high closes — with sonnet high/xhigh/max and opus high/xhigh this completes the stability datapoints; opus@max stays off. Quality-beyond-stability judged via: (a) mechanical payload checks now (pack_refs validity, A1-rule consistency, pack-utilisation rates — no spend), (b) concordance + error-direction as the registered accuracy measure in the one-pass computation across ALL arms once E8 v2 lands (Shawn's worksheet sitting remains the critical path), (c) optional small-sample fresh-context evidence-groundedness audit under the API gate if wanted | Shawn (rulings); Claude (execution) |
| 2026-08-29 | **F-010 RULED — path rule adopted** (Claude's recommendation endorsed): reconcile-run v1.5 judges an enumeration (Glob/Grep) by the paths it returned — truncated or unattributable results are unverifiable and fail — records a search root outside the allowed prefixes as a warning-grade `unscoped` enumeration, confines the hook-delivery exemption to Reads, and fixes the ≤v1.4 defect that recorded a Glob's base path in place of its pattern (**F-012, verifier-error**: crema r2 was an exact-filename existence check, not a repo-root listing; re-run on checker error). Replay of all 13 committed reconciliation reports under v1.5 with each report's own manifest vintage: 157 spawns, one verdict flip (crema r2 → clean), three re-described with the same outcome, zero receipt changes; committed reports untouched. dye r3 and key r2 attempt 2 restated: their `**/` globs surfaced an `archive/` duplicate of a skill reference — contaminating under both rules. Governance: refinement of what an "access" is under lodged amendment 2 §2, recorded in the register + here, no OSF amendment (addendum remains the registrant's option) | Shawn (ruling); Claude (build + replay) |
